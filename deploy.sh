...
NAS_COMPOSE_DIR="/volume1/docker/portal"  # Synology path where compose lives
NAS_SMB_DIR="/mnt/k/portal"              # Fallback: Windows SMB mount
...
echo -e "${CYAN}${BOLD}  🖥️ Portal — Deploy to Synology${RESET}"
if $DRY_RUN; then
...
if $HAS_SSH; then
  ...
  if $DRY_RUN; then
    ...
    info "  2. Project → portal → Stop → Start"
  fi
fi
