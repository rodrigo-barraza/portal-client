...
beforeEach(() => {
  ...
  api.restartContainer.mockResolvedValue(
    containerActionResponse("prism-service", "workstation"),
  );
});
...
describe("LogsComponent", () => {
  it("opens the deep-linked container on the linked host", async () => {
    vi.mocked(useSearchParams).mockReturnValue(
      new URLSearchParams(
        "container=prism-service&device=workstation",
      ) as never,
    );
    ...
    await waitFor(() =>
      expect(api.getContainerStats).toHaveBeenCalledWith(
        "workstation",
        expect.anything(),
      ),
    );
  ...
  it("shows the log text, not the JSON frame around it", async () => {
    ...
    await act(async () => {
      ...
      source.emit(
        "message",
        JSON.stringify({
          line: "2026-09-22T10:00:00Z server listening",
          stream: "stdout",
        }),
      );
  ...
  it("restarts only after confirmation, then follows the new stream", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Restart prism-service" }),
    );
    expect(api.restartContainer).not.toHaveBeenCalled();
    const dialog = screen.getByRole("alertdialog", {
      name: "Restart prism-service?",
    });
    await act(async () => {
      fireEvent.click(within(dialog).getByRole("button", { name: "Restart" }));
    });

    expect(api.restartContainer).toHaveBeenCalledWith(
      "prism-service",
      "synology",
    );
  ...
  it("says when the container list could not be loaded", async () => {
    ...
    render(<LogsComponent />);
    expect(
      await screen.findByText("Couldn't load containers: service down"),
    ).toBeInTheDocument();
  });
});
