...
describe("ExternalApisComponent", () => {
  it("never shows an older period's answer after a newer period was picked", async () => {
    ...
    expect(await screen.findByRole("button", { name: /Gemini 7d/ })).toBeInTheDocument();

    await act(async () => resolveThirty(summary("30d")));
    expect(screen.queryByRole("button", { name: /Gemini 30d/ })).not.toBeInTheDocument();
    expect(screen.getByRole("button", { name: /Gemini 7d/ })).toBeInTheDocument();
  });

  it("formats counts without rounding 1,500 up to 2K", async () => {
    api.getExternalApiUsageSummary.mockResolvedValue(summary("30d"));
    render(<ExternalApisComponent />);
    const card = (await screen.findByRole("button", { name: /Gemini 30d/ })).parentElement!;
  ...
  it("says when usage sources were unreachable instead of presenting partial totals", async () => {
    ...
    render(<ExternalApisComponent />);
    expect(await screen.findByText(/totals are incomplete: google-cloud-monitoring/)).toBeInTheDocument();
  });
  ...
  it("shows an error state with retry when the first load fails", async () => {
    api.getExternalApiUsageSummary.mockRejectedValueOnce(new Error("Monitoring API 403"));
    render(<ExternalApisComponent />);
    expect(await screen.findByText("Monitoring API 403")).toBeInTheDocument();

    api.getExternalApiUsageSummary.mockResolvedValue(summary("30d"));
    fireEvent.click(screen.getByRole("button", { name: "Retry" }));
    expect(await screen.findByRole("button", { name: /Gemini 30d/ })).toBeInTheDocument();
  });
});
