  ...
  return (
    <defs>
      {(Object.keys(EDGE_DIRECTION_CONFIG) as EdgeDirection[]).map(
        (direction) => {
          const { color, markerSize, markerOpacity } =
            EDGE_DIRECTION_CONFIG[direction];
          return (
            <marker
              key={direction}
              id={`arrow-${direction}`}
...
export const TopologyEdgePath = memo(function TopologyEdgePath({
  ...
  const directional =
    isSelected && direction ? EDGE_DIRECTION_CONFIG[direction] : null;
      ...
      <path
        ...
        strokeWidth={
          isActive
            ? 2.5
            : isOptional
              ? Math.max(edgeStyle.width - 0.5, 0.75)
              : edgeStyle.width
        }
        fill="none"
        strokeOpacity={
