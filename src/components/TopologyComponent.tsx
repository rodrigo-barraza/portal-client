...
import { AlertTriangle, Grid3x3, Layers, Maximize2, RefreshCw, ZoomIn, ZoomOut } from "lucide-react";
import {
  ButtonComponent,
  EmptyStateComponent,
  IconButtonComponent,
...
import type { EdgeType, NodePosition, PortalService, RepoSize } from "../types/portal";
import { defaultEdgeVisibility, EDGE_TYPE_CONFIG, EDGE_TYPES } from "./topology/topologyConfig";
import { buildClusterFrames, serviceTypeColor, type ClusterFrame } from "./topology/topologyFrames";
import {
  collectEdges,
  computeLayers,
  computeLibraries,
...
export default function TopologyComponent() {
  const { services: allServices, analysis, hasLoaded, error, isRefreshing, refresh } =
    useTopologyData();
  const {
    viewport,
    positionOverrides,
  ...
  const [hover, setHover] = useState<{ service: PortalService; x: number; y: number } | null>(
    null,
  );
  ...
  const filteredServices = useMemo(
    () => allServices.filter((service) => typeVisibility[service.projectType || "Other"] ?? true),
  ...
  const typeGroups = useMemo(() => computeTypeGroups(filteredServices), [filteredServices]);

  const basePositions = useMemo(
    () =>
      viewMode === "tier" ? layoutTierNodes(layers, libraries) : layoutTypeNodes(typeGroups),
  ...
  const edgeCounts = useMemo(() => {
    const counts = Object.fromEntries(EDGE_TYPES.map((type) => [type, 0])) as Record<EdgeType, number>;
  ...
  const activeSelection = selectedNode && positions[selectedNode] ? selectedNode : null;
  ...
  const clusterFrames = useMemo(
    () =>
      buildClusterFrames({ viewMode, layers, libraries, typeGroups, positions, searchMatches }),
    [viewMode, layers, libraries, typeGroups, positions, searchMatches],
  );
  const contentBounds = useMemo(() => nodeBounds(Object.keys(positions), positions), [positions]);
  ...
  const typeEntries = useMemo(() => {
    ...
    for (const service of allServices) if (service.projectType) types.add(service.projectType);
    return [...types].map((type) => ({ type, colors: serviceTypeColor(type) }));
  }, [allServices]);

  const healthyCount = filteredServices.filter((service) => service.healthy).length;
  ...
  const handleCanvasMouseDown = useCallback(
    (event: React.MouseEvent) => {
      ...
      if (element.closest("[data-topology-node]") || element.closest("[data-topology-cluster]")) {
        return;
      }
      setSelectedNode(null);
      beginPan(event);
  ...
  const handleNodePress = useCallback(
    (event: React.MouseEvent, service: PortalService, position: NodePosition) => {
      if (event.button !== 0) return;
  ...
  const handleClusterDragStart = useCallback(
    (event: React.MouseEvent, frame: ClusterFrame) => {
      ...
      for (const id of frame.memberIds) if (positions[id]) origins[id] = positions[id];
      beginClusterDrag(event, origins);
    },
    [beginClusterDrag, positions],
  );

  const handleHoverStart = useCallback((event: React.MouseEvent, service: PortalService) => {
  ...
  const toggleTypeVisibility = useCallback((type: string) => {
    setTypeVisibility((previous) => ({ ...previous, [type]: !(previous[type] ?? true) }));
  }, []);
    ...
    <div className={`topology-component ${styles["topology"]}`}>
          ...
          <div className={styles["header-text"]}>
            ...
            <p className={styles["subtitle"]}>
              ...
              {hasLoaded && github && github.status !== "ok" && (
                ...
                >
                  {" "}
                  · code analysis {github.status === "unavailable" ? "offline" : "degraded"}
                </span>
              )}
              {allServices.length > 0 && error && (
      ...
      {!hasLoaded ? (
        ...
        <EmptyStateComponent
          ...
          subtitle={error}
        >
          <ButtonComponent variant="secondary" icon={RefreshCw} loading={isRefreshing} onClick={refresh}>
            Retry
          </ButtonComponent>
        </EmptyStateComponent>
      ) : (
                ...
                {edges.map((edge) => {
                  ...
                  return (
                    <TopologyEdgePath
                      ...
                      source={source}
                      target={target}
                      edgeStyle={EDGE_TYPE_CONFIG[edge.type] || EDGE_TYPE_CONFIG.api}
                      isOptional={edge.criticality === "optional"}
                      isSelected={isSelected}
                      isHovered={hoveredId === edge.source || hoveredId === edge.target}
                      isFaded={isFaded}
                ...
                {filteredServices.map((service) => {
                  ...
                  return (
                    <TopologyNodeCard
                      ...
                      isFaded={
                        (activeSelection !== null && !connectedNodes.has(service.id)) ||
                        (searchMatches !== null && !searchMatches.has(service.id))
                      }
