...
describe("deviceModel", () => {
  it("groups containers per device, sorted by name", () => {
    ...
    expect(groups.nas.map((container) => container.name)).toEqual([
      "api",
      "web",
    ]);
  ...
  it("reads a device without containers as inactive, not down", () => {
    ...
    expect(deviceStatus(groupContainersByDevice(CONTAINERS).nas).variant).toBe(
      "unhealthy",
    );
    expect(deviceStatus(groupContainersByDevice(CONTAINERS).pi).variant).toBe(
      "healthy",
    );
  });
});
...
describe("DevicesComponent", () => {
  ...
  it("shows metrics only for running containers, colored by the user's thresholds", async () => {
    ...
    expect(cpuBadge.style.getPropertyValue("--metric-color")).toBe(
      "var(--color-warning)",
    );
  ...
  it("marks a device with no containers inactive and skips empty host rows", async () => {
    ...
    expect(
      screen
        .getByTitle("No containers reported")
