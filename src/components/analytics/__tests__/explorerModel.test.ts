...
describe("search filters", () => {
  const sessions = [
    ...
    session({ sessionId: "bbb", userId: "hello@rod.dev", geo: null, browser: null }),
  ];
  ...
  it("matches case-insensitively across fields", () => {
    expect(filterSessions(sessions, "FIREFOX").map((item) => item.sessionId)).toEqual(["aaa"]);
  });

  it("finds sessions by linked user identity", () => {
    expect(filterSessions(sessions, "rod.dev").map((item) => item.sessionId)).toEqual(["bbb"]);
  });

  it("tolerates null nested objects", () => {
    expect(() => filterSessions(sessions, "vancouver")).not.toThrow();
    expect(filterSessions(sessions, "vancouver").map((item) => item.sessionId)).toEqual(["aaa"]);
  });
  ...
  it("searches IPs by any linked visitor id", () => {
    ...
    expect(filterIpUsers(ips, "xyz").map((item) => item.ip)).toEqual(["198.51.100.1"]);
  });
  ...
  it("searches visitors by last IP", () => {
    ...
    expect(filterVisitors(visitors, "100.1").map((item) => item.visitorId)).toEqual(["v1"]);
  });
});
...
describe("buildTimeline", () => {
  it("merges chronologically and keeps each entry's session", () => {
    const timeline = buildTimeline(
      [
        { sessionId: "s2", url: "https://x/b", path: "/b", title: "B", timestamp: "2026-09-01T10:02:00Z" },
        { sessionId: "s1", url: "https://x/a", path: "/a", title: "A", timestamp: "2026-09-01T10:00:00Z" },
      ],
      [{ sessionId: "s1", category: "ui", action: "click", label: null, timestamp: "2026-09-01T10:01:00Z" }],
    );
  ...
  it("keeps page views ahead of events at the same instant", () => {
    const timeline = buildTimeline(
      ...
      [{ category: "c", action: "a", label: null, timestamp: "2026-09-01T10:00:00Z" }],
    );
...
describe("IP detail helpers", () => {
  it("rebuilds the cross-session timeline so rows keep their session tag", () => {
    const detail = ipDetail({
      pageViews: [{ sessionId: "s9", url: "u", path: "/", title: null, timestamp: "2026-09-01T10:00:00Z" }],
      // The service's merged timeline has no sessionId
      timeline: [{ type: "pageview", timestamp: "2026-09-01T10:00:00Z", path: "/" }],
    });
    expect(ipTimeline(detail)[0].sessionId).toBe("s9");
  });

  it("falls back to the service timeline when raw records are absent", () => {
    const timeline = [{ type: "event" as const, timestamp: "2026-09-01T10:00:00Z" }];
